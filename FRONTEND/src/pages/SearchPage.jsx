...
const SearchPage = () => {
  ...
  const query = SearchParams.get("query"); // On récupère la valeur du paramètre
  const [montres, setMontres] = useState([]); // State pour stocker les résultats

  useEffect(() => {
    if (query) {
      api
        .get(`/montres/search?query=${query}`)
  ...
  return (
    <div className="container mt-5">
      ...
      {montres.length === 0 ? (
        ...
        <div className="row">
          {montres.map((montre) => (
            <div key={montre.id} className="col-12 col-sm-6 col-md-4 mb-3">
              <Link
