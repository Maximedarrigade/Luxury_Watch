...
export const CartProvider = ({children}) => {
    ...
    const addPanier = (montre) => {
        ...
        if(exist) {
            ...
            setPanier(panier.map((item) => item.id === montre.id ? {...item, quantite: item.quantite + 1} : item))
    ...
    const deletePanier = (id) => {
        ...
        setPanier(panier.map((item) => item.id === id ? {...item, quantite: item.quantite - 1} // Diminution de la quantité de -1
        
        : item).filter((item) => item.quantite > 0)); // On supprime si la quantité arrive a 0
    ...
    return (
        ...
        <CartContext.Provider value={{panier, addPanier, deletePanier, viderPanier, total, nombreArticles}}>
